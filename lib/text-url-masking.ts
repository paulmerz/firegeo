...
export function maskUrlsInText(text: string, hideSources: boolean): string {
  ...
  // 5. Supprimer complètement les domaines entre parenthèses: (example.com), (vogue.co.uk)
  ...
  // Nettoyer les espaces multiples et ponctuations orphelines
  maskedText = maskedText.replace(/\s+,\s+/g, ', '); // Espaces autour des virgules
  maskedText = maskedText.replace(/\s+/g, ' '); // Espaces multiples
  maskedText = maskedText.replace(/,\s*\./g, '.'); // Virgule avant point
  maskedText = maskedText.replace(/\s*,\s*$/gm, ''); // Virgule en fin de ligne
